...
archives_cdx_api = 'http://web.archive.org/cdx/search/cdx'
archives_web_api = 'https://web.archive.org/web'
memento_api = 'http://timetravel.mementoweb.org/timemap/json'
default_limit = 1
...
def query_url(url, limit, match_codes):
    ...
    response = requests.get(url=archives_cdx_api, params=params, timeout=default_timeout)
    response.raise_for_status()  
    data = response.json()
...
def parse_snapshot(record, raw = True):
    ...
    return f'{archives_web_api}/{record_timestamp}/{record["original"]}'

def query_batch(urls, limit = 15, match_codes=None):
    for url in urls:
...
def parse_args():
    ...
    parser.add_argument('-u', '--url', type=str, help='Fetch snapshots for single URL')
    parser.add_argument('-f', '--file', type=FileType('r'), default='-', nargs='?', help='Fetch snapshots for multiple URLS in file')
    parser.add_argument('-l', '--limit', type=int, default=default_limit, help='Limit number of snapshots fetched per URL')
